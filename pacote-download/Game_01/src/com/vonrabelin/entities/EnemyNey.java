...
public class EnemyNey extends Enemy{

	private double speed = 0.8;
	private int maskx = 6, masky = 3, maskw = 10, maskh = 10; // posição de x, y, largura e altura da máscara	
	private static int frames = 0, maxFrames = 900, index = 0, maxIndex = 1;
	
	private BufferedImage[] sprites;
	
	private int life = 1; // 8
	
	private boolean isDamaged = false;
	private int damageFrames = 10, damageCurrent = 0;
	
	public EnemyNey(int x, int y, int width, int height, BufferedImage sprite) {
		super(x, y, width, height, null);
		sprites = new BufferedImage[2];
		sprites[0] = Entity.ENEMY_NEY;
		sprites[1] = Entity.ENEMY_NEY_2;
	}
	...
	public void render(Graphics g) {
		...
		if (!isDamaged) {
			...
			g.drawImage(Entity.ENEMY_NEY_DAMAGE, this.getX() - Camera.x, this.getY() - Camera.y, null);
		}
